"""Set up the fraud_demo database, collections, and vector search index.

Performs the equivalent of plan 01 sections 1.3 and 1.4:
    1.3  use fraud_demo
         db.createCollection("transactions")
         db.createCollection("fraud_examples")
    1.4  create fraud_vector_idx (Atlas Vector Search on fraud_examples.embedding)

Idempotent — safe to re-run. Run from the project root (fraud-detect/):
...
DB_NAME = "fraud_demo"
COLLECTIONS = ["transactions", "fraud_examples"]
VECTOR_COLLECTION = "fraud_examples"
VECTOR_INDEX_NAME = "fraud_vector_idx"

VECTOR_INDEX_DEFINITION = {
    "fields": [
        {"type": "vector", "path": "embedding", "numDimensions": 1024, "similarity": "cosine"},
        {"type": "filter", "path": "archetype"},
    ]
}
...
def setup_database(client):
    """Plan section 1.3: ensure the database and its collections exist.

    MongoDB creates databases lazily on first write, so 'use fraud_demo' alone
...
def setup_indexes(collection):
    """Plan section 1.4: create the Vector Search index on fraud_examples."""
