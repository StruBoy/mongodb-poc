"""Application-layer field encryption for sensitive PHI.

The PoC uses AES-256-GCM at the application layer for clarity — the encrypt /
decrypt calls sit right next to the application code that handles PHI, which
...
PHI_FIELDS = {"national_id", "dob", "insurance_id"}
...
def encrypt_phi(doc: dict) -> dict:
    out = {}
    for k, v in doc.items():
        if k in PHI_FIELDS and v is not None and not _is_encrypted_envelope(v):
...
def decrypt_phi(doc: dict) -> dict:
    if doc is None:
        return None
    out = {}
    for k, v in doc.items():
        if k in PHI_FIELDS and _is_encrypted_envelope(v):
